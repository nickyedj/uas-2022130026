...
@section('content')
                ...
                <tbody>
                    @foreach ($songs as $song)
                        <tr data-song-id="{{ $song->id }}" onclick="playSong({{ $song->id }})">
                            ...
                            <td>
                                <form action="{{ route('playlist.addSong', $song->id) }}" method="POST"
                                    style="display:inline;">
                                    @csrf
                                    <select name="playlist_id" class="form-control form-control-sm">
    ...
    <script>
        function playSong(songId) {
            ...
            // Mengirim data ke server (misalnya untuk melacak lagu yang didengarkan)
            fetch(`/songs/${songId}/listen`, {
                ...
                .then(data => {
                    console.log('Server response:', data); // Cek respons dari server
                })
